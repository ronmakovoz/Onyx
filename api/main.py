...
@app.post("/api/agents/run-red-team-debate")
def red_team_debate(customer_id: int):
...
@app.get("/api/customers/{customer_id}/debate", response_class=HTMLResponse)
def debate_view(customer_id: int, fresh: bool = False):
    """Branded HTML view of the latest Red Team Debate for a customer.

    Serves the cached debate instantly when one exists; otherwise (or with
    ?fresh=1) serves a loading page that runs the debate from the browser
    and reloads — bull/bear/synthesis can take a couple of minutes live.
    """
    from dataclasses import asdict
            ...
            for key, run, agent_name in (("bull", bull, "BullCaseAgent"),
                ...
                parts[key] = {
                    **dict(run),
                    ...
                    "model_display": _route(agent_name).model_display,
                }
