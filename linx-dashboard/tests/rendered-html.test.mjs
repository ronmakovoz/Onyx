...
test("separates the Linx platform foundation from customer-facing solutions", async () => {
  ...
  assert.match(productMap, /An agentless, API-based path/);
  assert.match(productMap, /systems remain the source of truth/);
  assert.match(productMap, /Four steps from connection to outcome/);
  assert.match(productMap, /no inbound firewall rule or installed agent/);
  assert.match(productMap, /OAuth \+ REST/);
  assert.match(productMap, /REST \/ SCIM/);
  assert.match(productMap, /SAML \/ OIDC/);
  assert.match(productMap, /Customer IdP → Linx/);
  assert.match(productMap, /Write access is a separate consent step/);
