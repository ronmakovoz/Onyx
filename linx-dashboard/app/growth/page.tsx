...
export default function GrowthPage() {
  ...
  return (
    <div>
      <PageHeader title="Growth & Whitespace" subtitle="What customers own, what similar identity programs adopt next, and the ARR opportunity" />
        ...
        {accounts.map((account) => (
          <button key={account.id} className={`matrix-row ${selected.id === account.id ? "selected" : ""}`} onClick={() => setSelectedId(account.id)}>
            ...
            {products.map((product) => {
              ...
              const recommendation = !owned && account.expansion > 0 && ["jit", "nhi", "ai-access", "autopilot", "lifecycle"].includes(product.id);
      ...
      <section className="card whitespace-detail">
        ...
        ))}</div> : <p className="empty-state">This account is fully penetrated across the modeled Linx platform.</p>}
      </section>
    </div>
  );
}
