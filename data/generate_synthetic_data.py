...
MASTER_CUSTOMERS = [
    ( 1, "NovaPay",                 "Financial Services",  "NA",    4200,   980000,  -300,  52, "Mature",     86, "Improving", "Low",    "Shadow AI discovery ROI proven; expansion underway",     "Prep multi-product expansion proposal"),
    ( 2, "Sentinel Commerce",       "Ecommerce",           "NA",    3100,   540000,  -210,  88, "Mature",     79, "Stable",    "Low",    "Strong runtime-protection results; healthy adoption",    "Introduce AI-SPM module"),
    ( 3, "Orbit Travel Group",      "Travel",              "EMEA",  6800,   720000,  -180,  34, "At Risk",    41, "Declining", "High",   "Guardian policy drift on booking agents; champion disengaged", "Policy tuning workshop + exec call"),
    ...
    ( 7, "Velocity Marketplace",    "Marketplace",         "NA",    3400,   610000,  -150,  29, "At Risk",    44, "Declining", "High",   "Guardian false positives spiking on seller agents; trust eroding", "Tuning war room + daily updates"),
    ( 8, "BluePeak Retail",         "Retail",              "NA",    7600,   380000,  -365, 178, "Mature",     76, "Stable",    "Low",    "Steady usage; light upsell available",                   "Pitch AI ROI analytics add-on"),
    ( 9, "Apex Interactive",        "Gaming",              "APAC",  1900,   470000,  -120,  47, "Onboarding", 35, "Declining", "High",   "Go-live stalled; MCP server onboarding blocked",         "Engineering escalation on integration"),
    (10, "VitalCore Health",        "Healthcare",          "NA",    3200,   690000,  -200,  72, "Mature",     69, "Stable",    "Medium", "Adoption plateau; PHI guardrail workflow friction",      "Workflow optimization session"),
    ...
    (12, "LuminaCare Medical",      "Healthcare",          "EMEA",   890,   360000,   -55,   8, "Onboarding", 27, "Declining", "High",   "PHI exposure concern in AI agents blocking go-live; CISO disengaged", "Legal + security review escalation"),
    (13, "ShopSphere",              "Ecommerce",           "APAC",  4100,   520000,  -300, 134, "Mature",     81, "Improving", "Low",    "LLM routing cost savings growing; new champion",         "Schedule platform deep-dive"),
    (14, "NovaTech Systems",        "Enterprise Technology","NA",   2200,   760000,  -280,  61, "Mature",     66, "Stable",    "Medium", "CNAPP discovery-source integration gap; roadmap request pending", "Engineering roadmap sync"),
    ...
    (16, "Sterling Trust Bank",     "Financial Services",  "NA",    8900,  1600000,  -500, 156, "Mature",     89, "Stable",    "Low",    "AI governance expanded; strong compliance use",          "Annual exec business review"),
    ...
    (18, "WanderStay Hotels",       "Travel",              "EMEA",  3100,   410000,  -100,  19, "At Risk",    33, "Declining", "High",   "Shadow AI coverage gap; champion left",                  "Identify and engage new champion"),
    ...
    (21, "AssureGuard Insurance",   "Insurance",           "NA",    5200,   930000,  -730, 118, "Mature",     87, "Stable",    "Low",    "AI agent governance ROI strong; compliance aligned",     "Expand to underwriting division"),
    ...
    (23, "Meridian Capital",        "Financial Services",  "EMEA",  3400,   880000,  -320,  83, "Mature",     78, "Improving", "Low",    "Runtime-protection expansion live",                      "Plan phase-2 rollout"),
    ...
    (31, "PixelForge Games",        "Gaming",              "EMEA",  2200,   350000,  -260,  88, "Mature",     72, "Improving", "Low",    "Prompt-injection blocking proven; growing usage",        "Upsell LLM routing add-on"),
    ...
    (34, "GlobalEdge Retail",       "Retail",              "EMEA",  5500,   290000,  -420, 168, "Mature",     73, "Stable",    "Low",    "Renewal secured; light upsell opportunity",              "Introduce AI-SPM module"),
    (35, "Cobalt Payments",         "Financial Services",  "NA",    3900,   810000,  -140,  48, "At Risk",    47, "Declining", "High",   "Runtime policy-engine latency complaints; trust at risk","Performance remediation plan"),
    (36, "BazaarOne",               "Marketplace",         "APAC",  4200,   560000,  -340,  98, "Mature",     76, "Improving", "Low",    "Agent governance adoption rising",                       "Expand to logistics AI agents"),
    ...
    (40, "Auric Bank",              "Financial Services",  "APAC",  6100,  1050000,  -380,  92, "Mature",     85, "Stable",    "Low",    "Multi-region AI observability coverage strong",          "Discuss regional expansion"),
    ...
    (42, "Lumen Insurance Group",   "Insurance",           "EMEA",  4800,   700000,  -160,  44, "At Risk",    49, "Declining", "High",   "Guardian Agent precision underperforming; renewal risk", "Policy performance deep-dive"),
    (43, "PlaySphere",              "Gaming",              "NA",    1100,   210000,  -300, 120, "Mature",     71, "Improving", "Low",    "Runtime protection adopted well",                        "Offer expansion discount"),
    ...
    (46, "Crestline Capital",       "Financial Services",  "NA",    2400,   590000,  -270,  90, "Mature",     82, "Improving", "Low",    "Compliance evidence automation delivering value",        "Expand to AI-SPM module"),
    (47, "ZenMarket",               "Marketplace",         "EMEA",  3600,   440000,  -100,  28, "At Risk",    43, "Declining", "High",   "AI governance team overwhelmed; Guardian false positives high","Tuning engagement + exec review"),
    (48, "Summit Health Plan",      "Insurance",           "NA",    5400,   770000,  -330,  96, "Mature",     79, "Stable",    "Low",    "LLM spend savings documented",                           "Build expansion business case"),
    (49, "Pinnacle Gaming",         "Gaming",              "APAC",  2600,   400000,  -150,  35, "At Risk",    46, "Declining", "Medium", "Shadow AI discovery coverage gap; mid-tier risk",        "Coverage gap remediation plan"),
...
# Realistic, measurable AI-governance business outcomes by industry. Strong
# outcomes are reserved for healthier accounts to keep the data internally
# consistent (high adoption -> demonstrable ROI).
ROI_OUTCOMES = {
    "Financial Services": ["LLM spend cut {a}% via intelligent routing", "AI governance review queue cut {b}%", "Guardian false positives lowered {c}%", "AI compliance evidence prep time down {b}%"],
    "Ecommerce":          ["Shadow AI apps remediated: {e}", "Prompt-injection attempts blocked on customer-facing agents: {e}", "Manual AI risk reviews reduced {b}%", "Agent task success rate up {d}%"],
    "Retail":             ["LLM spend reduced {a}% via routing", "Guardian interventions triaged {b}% faster", "Guardian false positives lowered {c}%"],
    "Travel":             ["Data-exfiltration attempts blocked: {e}", "Ungoverned agent incidents down {a}%", "Manual AI reviews reduced {b}%"],
    "Marketplace":        ["MCP supply-chain risks remediated: {e}", "AI governance review load down {b}%", "Guardian false positives lowered {c}%"],
    "Healthcare":         ["PHI exposure attempts blocked by Guardian: {e}", "HIPAA AI evidence prep time reduced {b}%", "Shadow AI apps discovered: {e}"],
    "Insurance":          ["AI agents brought under policy enforcement: {a}% more", "Compliance analyst hours saved {b}%", "Discovered {e} shadow AI apps in first scan"],
    "Gaming":             ["Jailbreak attempts blocked {a}% more effectively", "LLM spend reduced {a}% via routing", "Manual AI reviews reduced {b}%"],
    "SaaS":               ["Prompt-injection attempts blocked: {e}", "Platform team time saved {b}%", "Guardian false-positive rate lowered {c}%"],
    "Enterprise Technology": ["Shadow AI apps discovered: {e}", "Mean-time-to-remediate AI risk reduced {b}%", "Platform team time saved {b}%"],
}
...
TICKET_POOL = {
    "P1": [
        ...
        "AI activity log ingestion stopped — 6-hour gap in coverage",
        "Guardian Agent blocking legitimate agent actions in production",
        "Runtime policy engine latency spiking — agent calls timing out",
        "API gateway returning 500 errors on all requests",
        "PHI data appearing in unencrypted agent-trace export",
        "Runtime protection bypassed after platform update",
        "LLM routing gateway silent for 8+ hours — all agent traffic failing over",
    ],
    "P2": [
        "SSO integration failing intermittently (15% failure rate)",
        "API rate limits too restrictive for enterprise agent workloads",
        ...
        "Custom Guardian policy not triggering on known prompt-injection pattern",
        "MCP server discovery missing self-hosted servers",
        "EU AI Act compliance report export missing 3 required fields",
        "Prompt-injection false positives on internal copilot",
    ],
    "P3": [
        "Guardian intervention noise too high — estimated 60% false positive rate",
        "Unable to export AI compliance reports in PDF format",
        "Slow query performance on agent traces > 1M events",
        "UI session timeout too aggressive (15 minutes)",
        "Missing pagination on shadow AI discovery results",
        "AI activity log search returning inconsistent results",
        "Mobile app notification delays > 10 minutes",
        "CSV export truncating fields over 256 characters",
        "Duplicate entries in agent activity report",
        "Dashboard widget not refreshing on Safari",
    ],
    "P4": [
        "Feature request: bulk user import via CSV",
        "Documentation missing for advanced Guardian policy syntax",
        "Request to add dark mode to governance console",
...
ALL_MILESTONES = [
    ("Kickoff & Scoping",               7),
    ("Discovery Sources Connected",     14),
    ("Shadow AI Baseline Scan",         21),
    ("SSO / Identity Provider Integration", 35),
    ("MCP Server Inventory & Onboarding", 42),
    ("Pilot Group Onboarding (25 users)", 56),
    ("Guardian Agent Policy Rollout",   63),
    ("Integration Testing & QA",        77),
    ("Runtime Protection Enabled",      84),
...
POSITIVE_NOTES = [
    "Customer expressed strong satisfaction with Guardian Agent precision. NPS 9 shared unprompted. Champion confirmed willingness to be a reference.",
...
NEUTRAL_NOTES = [
    ...
    "Meeting covered usage review and upcoming feature releases. Customer noted they haven't fully activated runtime protection.",
    "Customer asked about SOC2 report availability. Shared compliance documentation. No concerns raised.",
    "Discussed connecting their CNAPP as a discovery source. Customer IT team needs 2 weeks to schedule the work. No escalation needed.",
