...
export default function GrowthPage() {
  ...
  return (
    <div>
      <PageHeader title="Growth & Whitespace" subtitle="What partners use, what similar property portfolios adopt next, and the ARR opportunity" />
      ...
      <SectionTitle meta="filled = active · ring = recommended">Resident experience penetration</SectionTitle>
        ...
        {accounts.map((account) => (
          <button key={account.id} className={`matrix-row ${selected.id === account.id ? "selected" : ""}`} onClick={() => setSelectedId(account.id)}>
            ...
            {products.map((product) => {
              ...
              const recommendation = !owned && account.expansion > 0 && ["onboarding", "maestro", "internet", "credit", "filters", "rewards", "concierge"].includes(product.id);
      ...
      <section className="card whitespace-detail">
        ...
        ))}</div> : <p className="empty-state">This account is fully activated across the modeled Second Nature resident experience platform.</p>}
      </section>
    </div>
  );
}
